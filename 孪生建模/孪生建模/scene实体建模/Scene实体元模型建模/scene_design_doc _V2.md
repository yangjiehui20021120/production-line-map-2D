...
在制造过程中,物料会经历多个加工状态,每个状态都应该建模为独立的TwinObject实例:

**示例:侧墙加工工序**
```
输入: urn:ngsi-ld:TwinObject:Workpiece.PO123-001-AfterWelding (焊接后侧墙)
  ↓ [加工Scene执行]
输出: urn:ngsi-ld:TwinObject:Workpiece.PO123-001-AfterMachining (加工后侧墙)
  ↓ [后续工序...]
最终: urn:ngsi-ld:TwinObject:Product.PO123-001-Final (最终产品)
```
...
```json
{
  "id": "urn:ngsi-ld:Scene:Process:PO123-001:T01-P0020:20250108150000",
  "type": "Scene",
  "sceneLevel": {"type": "Property", "value": "Process"},
  "sceneName": {"type": "Property", "value": "侧墙精加工工序"},
  
  "inputWorkpiece": {
    "type": "Relationship",
    "object": "urn:ngsi-ld:TwinObject:Workpiece.PO123-001-AfterWelding",
    "observedAt": "2025-01-08T15:00:00Z"
  },
  
  "outputWorkpiece": {
    "type": "Relationship",
    "object": "urn:ngsi-ld:TwinObject:Workpiece.PO123-001-AfterMachining",
    "observedAt": "2025-01-08T16:30:00Z"
  },
  ...
  "consumedMaterials": {
    ...
    "value": [
      {
        "materialId": "urn:ngsi-ld:TwinObject:Consumable.CuttingFluid_Batch456",
...
**案例2: 装配类工序(多输入组合成一个输出)**
```json
{
  "id": "urn:ngsi-ld:Scene:Process:PO123-001:T02-P0035:20250108170000",
  "type": "Scene",
  "sceneLevel": {"type": "Property", "value": "Process"},
  "sceneName": {"type": "Property", "value": "侧墙附件焊接装配工序"},
  
  "inputWorkpiece": {
    "type": "Relationship",
    "object": "urn:ngsi-ld:TwinObject:Workpiece.PO123-001-AfterFinishing",
    "description": "主工件:精细打磨后侧墙",
    "observedAt": "2025-01-08T17:00:00Z"
  },
  ...
  "inputMaterials": {
    ...
    "value": [
      {
        "materialId": "urn:ngsi-ld:TwinObject:Material.Nut_M8_Batch789",
        "materialName": "M8螺母",
        "quantity": 24,
        "unit": "piece",
        "batchNo": "LOT-20250105-001",
        "supplierCode": "SUP-A001"
      },
      {
        "materialId": "urn:ngsi-ld:TwinObject:Material.Bracket_Type3_Serial123",
        "materialName": "3型支架",
        "quantity": 4,
        "unit": "piece",
        "batchNo": "LOT-20250106-002",
        "supplierCode": "SUP-B002"
      },
      {
        "materialId": "urn:ngsi-ld:TwinObject:Material.Connector_TypeC_Serial456",
        "materialName": "C型连接件",
        "quantity": 8,
        "unit": "piece",
        "batchNo": "LOT-20250107-003",
  ...
  "outputWorkpiece": {
    "type": "Relationship",
    "object": "urn:ngsi-ld:TwinObject:Workpiece.PO123-001-WithAccessories",
    "description": "装配附件后侧墙",
    "observedAt": "2025-01-08T19:30:00Z"
  },
  
  "refFinalProduct": {
    "type": "Relationship",
    "object": "urn:ngsi-ld:TwinObject:Product.PO123-001-Final",
    "description": "追溯到最终交付产品"
  },
  
  "involvesAsset": {
    "type": "Relationship",
    "object": [
      "urn:ngsi-ld:TwinObject:Robot.WELD-R02",
      "urn:ngsi-ld:TwinObject:Fixture.FIX-ASM-01",
      "urn:ngsi-ld:TwinObject:Person.Li_Si"
    ]
  }
}
```
...
物料在Scene间的流转通过MoveIn/MoveOut事件记录:

```json
{
  "id": "urn:ngsi-ld:Event:MoveIn:Scene:Process:...:20250108150000",
  "type": "Event",
  "eventType": {"type": "Property", "value": "MoveIn"},
  "refScene": {
    "type": "Relationship",
    "object": "urn:ngsi-ld:Scene:Process:PO123-001:T01-P0020:..."
  },
  "refWorkpiece": {
    "type": "Relationship", 
    "object": "urn:ngsi-ld:TwinObject:Workpiece.PO123-001-AfterWelding"
...
**设计优势**:
1. **完整追溯**: 从最终产品可以追溯到所有中间态物料和原材料
2. **状态清晰**: 每个加工状态都有独立实体,状态变化明确
3. **质量定位**: 质量问题可精确定位到具体加工阶段的物料或原材料批次
4. **物理流重建**: 通过MoveIn/MoveOut事件可重建完整物理流转路径
5. **支持返工**: 如果需要返工,可以创建新的物料实例,不影响原有追溯链
6. **装配追溯**: 对于装配工序,可以追溯到每个安装件的批次和供应商
7. **质量召回**: 如发现某批次原材料有问题,可快速定位所有使用该批次的产品
...
1. **流转性在制品 (Workpiece)**
   - 特点: 唯一ID,随工序流转,状态不断演进
   - 示例: "焊接后侧墙" → "精加工后侧墙" → "打磨后侧墙"
   - 建模: 每个状态都是独立的TwinObject实例

2. **消耗性原材料/采购件 (Material/Component)**
   - 特点: 按批次管理,可能一次消耗多个
   - 示例: 螺母、螺栓、配件、焊丝、切削液等
   - 建模: 
     - 可以是批次级TwinObject (如 `Material.Nut_M8_Batch789`)
     - 也可以是序列化个体 (如 `Material.Bracket_Serial123`)
     - 关键是记录批次号、供应商等追溯信息
...
在实际生产中,物料会经历多个状态变化,每个状态都应该是独立的TwinObject实例:

```json
{
  "inputWorkpiece": {
    "type": "Relationship",
    "object": "urn:ngsi-ld:TwinObject:Workpiece.PO123-001-AfterAdjustment"
  },
  "outputWorkpiece": {
    "type": "Relationship", 
    "object": "urn:ngsi-ld:TwinObject:Workpiece.PO123-001-AfterMachining"
...
**设计原则**:
- 输入物料和输出物料都是独立的TwinObject实例,各有唯一ID
- 通过MoveIn/MoveOut事件记录物料在Scene间的流转
- 所有中间态物料通过关系链追溯到最终产品
- 支持完整的物理流追踪和质量追溯
